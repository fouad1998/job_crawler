...
from helpers import (get_clipboard_text, html_to_formatted_text, parse_json,
                     remove_markers)
from prints import job_updated
        ...
        while True:
            ...
            with semaphore:
                ...
                try:
                    ...
                    cookie_banner_element = sb.cdp.find_element(cookie_banner_selector, timeout=15)
                    cookie_banner_element.click()
                except:
                    print("explorer: Cookie banner not found")
                ...
                try:
                    ...
                    try:
                        print("explorer: Error in getting job description:", error)
                        page_removed_indicator_text = "Cette offre d'emploi n’est plus disponible."
                        sb.cdp.find_elements_by_text(page_removed_indicator_text)
                        print("explorer: Job removed")
            ...
            while not event.is_set() or not answers_queue.empty():
                try:
                    ...
                    if chatgpt_answer_values is None:
                        raise "Explorer thread: ChatGPT answer values is None"
                    ...
                    job = dict(chatgpt_answer_values)
                    job["title"] = job_title
                    job_updated(job)
                    ...
                    print("Explorer: Error in Explorer:", error)
                    event.set()
                    raise error
            ...
            try:
                ...
                try:
                    with semaphore:
                        ...
                        # wait for the last article to be loaded
                        chatgpt_answer_values = None
                        current_attempts_searching_json = 0
                        ...
                        while True:
                            ...
                            # Get articles length
                            articles_length = len(chatgpt_discussion_container.query_selector_all("article"))
                            if articles_length == discussion_count*2:
                                # Go to view port of the last article
                                chatgpt_last_article = chatgpt_discussion_container.query_selector_all("article")[-1]
                                chatgpt_last_article.scroll_into_view()
                                print("Chatgpt: Answer generated")
                                
                                # pre element in last article
                                chatgpt_last_article_pre = chatgpt_last_article.query_selector("pre")
                                if chatgpt_last_article_pre is not None:
                                    # json code element in pre element
                                    chatgpt_last_article_json = chatgpt_last_article_pre.query_selector("& > div > div:last-child")
                                    if chatgpt_last_article_json is not None:
                                        ...
                                        print("chatgpt: attempt to parse json data")
                                        chatgpt_answer_values = parse_json(chatgpt_answer)
                                        if chatgpt_answer_values is not None:
                                            break;
                            
                            current_attempts_searching_json += 1
                            print("Chatgpt: Waiting for the answer to be generated")
                            if chatgpt_answer_values is None and current_attempts_searching_json > 50:
                                raise "Chatgpt: The answer is not generated"
